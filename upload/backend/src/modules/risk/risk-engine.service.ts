...
export class RiskEngineService {
  ...
  assess(input: RiskInput): RiskAssessmentResult {
    ...
    const forecast = Array.isArray(input.weather.forecastData) ? input.weather.forecastData : [];
    const rainyDays = forecast
      .slice(0, 3)
      .filter((d) => {
        const icon = String(d.icon ?? '').toLowerCase();
        const condition = String(d.condition ?? '').toLowerCase();
        return icon.includes('rain') || condition.includes('rain') || condition.includes('雨');
      }).length;
    ...
    const daysWithoutIrrigation = input.latestIrrigationAt
      ? Math.floor((input.now.getTime() - input.latestIrrigationAt.getTime()) / (1000 * 60 * 60 * 24))
      : 99;
    ...
    if (temp >= 32 && daysWithoutIrrigation >= 5) {
      items.push({
        ...
        title: '干旱胁迫风险',
        reason: `当前温度约${temp}℃，且已${daysWithoutIrrigation}天无灌溉记录，存在水分胁迫风险。`,
        score: temp >= 35 ? 35 : 24,
      });
    }
    ...
    if (input.recentDiseaseCount >= 2) {
      items.push({
        ...
        level: input.recentHighDiseaseCount >= 1 ? RiskLevel.high : RiskLevel.mid,
        title: '病害历史风险',
        reason:
          input.recentHighDiseaseCount >= 1
            ? `近7天识别到${input.recentDiseaseCount}条病害记录，包含高风险样本。`
            : `近7天识别到${input.recentDiseaseCount}条病害记录，需要加强巡园。`,
    ...
    const overallLevel =
      overallScore >= 60 ? RiskLevel.high : overallScore >= 30 ? RiskLevel.mid : RiskLevel.low;

    return {
      overallLevel,
      overallScore,
      riskItems: items,
      suggestions: this.buildSuggestions(items, overallLevel),
    };
  }
  ...
  private buildSuggestions(items: RiskItem[], overall: RiskLevel) {
    ...
    for (const item of items) {
      if (item.type === 'disease-spread' || item.type === 'disease-history') {
        ...
        set.add('对可疑区域进行复拍复核，必要时进行针对性防治。');
      }
